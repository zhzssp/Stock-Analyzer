...
│   ├── 软件设计文档.md        # 【核心】两大功能、选型、扩展与分期
│   ├── 设计文档.md            # 多智能体与插件契约专论
│   ├── 对客功能说明.md        # 按当前方案，用户能用到什么
│   ├── frontend/              # 前端设计稿与可点击原型
│   ├── 验证报告.md            # 由 Verify-Api.ps1 生成
│   └── 指数成份探针.md        # 由 Verify-Index.ps1 生成（S0/S6）
├── pics/                     # 现有表格截图，字段比对的基准
...
### [`docs/软件设计文档.md`](docs/软件设计文档.md)

当前主设计：传统查询（图 1 字段 + Excel 导出）与 Agent 系统（API/历史表为 Tool、问答与定时监控）并列；含前后端与 LangGraph 选型、扩展槽位和分期。

### [`docs/设计文档.md`](docs/设计文档.md)

多智能体与插件契约专论。实现细节以软件设计文档为准。

### [`docs/对客功能说明.md`](docs/对客功能说明.md)

按当前方案落地后，用户能直接用到的功能：分析表、问答、盯盘提醒；以及明确不会做的事。
