...
.DESCRIPTION
    针对 docs/API/API现状.md 中 §4 列出的「已知限制与坑」逐条实测，
...
.PARAMETER OutFile
    报告输出路径。默认为仓库内 docs/reports/验证报告.md
...
if ([string]::IsNullOrWhiteSpace($OutFile)) {
    $OutFile = Join-Path $RepoRoot 'docs\reports\验证报告.md'
}
...
Add-Line "# API 验证报告"
Add-Line ""
Add-Line "> 由 ``scripts/Verify-Api.ps1`` 自动生成，用于实测校验 ``docs/API/API现状.md`` §4 的各项判断。"
Add-Line ">"
